...
class ChangeRequestTableAccess {
    fun getAll(): List<ChangeRequest> = transaction {
        ChangeRequestTable.selectAll().map { it.toChangeRequest() }
    }
    fun <T> getByAttribute(attribute: Column<T>, value: T): List<ChangeRequest> = transaction {
        ChangeRequestTable
            .select { attribute eq value }
            .map { it.toChangeRequest() }
    }
    fun findById(id: Int): ChangeRequest? = transaction {
        ChangeRequestTable
    ...
    fun <T> updateRecordByAttribute(id: Int, column: Column<T>, value: T): Boolean = transaction {
        val rows = ChangeRequestTable.update({ ChangeRequestTable.id eq id }) { stmt ->
            stmt[column] = value
            stmt[ChangeRequestTable.updatedAt] = Instant.now().toString()
        }
        rows > 0 }
    fun updateStatus(id: Int, status: String): Boolean = transaction {
        val rows = ChangeRequestTable.update({ ChangeRequestTable.id eq id }) {
